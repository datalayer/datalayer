...
PS: An image `elyra/nb-kernel-mgmt:dev` containing the `notebook`, `jupyter_kernel_mgmt`, `remote_kernel_provider`, `kubernetes_kernel_provider` and `docker_kernel_provider` whl and `kernelspec` files can be found [here](https://hub.docker.com/r/elyra/nb-kernel-mgmt).
