...
public class PodCIDRToNodeMapping extends AbstractDNSToSwitchMapping {
   ...
   * a pod CIDR. For instance, node A can be assigned 10.0.0.0/24 while node B gets 1${DLAVERSION}.0/24.
   * When a pod has an IP value, say 1${DLAVERSION}.10, it should match node B.
   *
   ...
   *    cluster node. e.g. 10.0.0.0 -> node A and 1${DLAVERSION}.0 -> node B.
   */
