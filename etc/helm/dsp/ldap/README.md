...
==> v1/ConfigMap
NAME               DATA  AGE
dsp-ldap-openldap-env  6     0s

==> v1/Pod(related)
NAME                            READY  STATUS             RESTARTS  AGE
dsp-ldap-openldap-7dc7579d5d-m5292  0/1    ContainerCreating  0         0s

==> v1/Secret
NAME           TYPE    DATA  AGE
dsp-ldap-openldap  Opaque  2     0s

==> v1/Service
NAME           TYPE       CLUSTER-IP      EXTERNAL-IP  PORT(S)          AGE
dsp-ldap-openldap  ClusterIP  10.100.189.233  <none>       389/TCP,636/TCP  0s

==> v1beta2/Deployment
NAME           READY  UP-TO-DATE  AVAILABLE  AGE
dsp-ldap-openldap  0/1    1           0          0s

NOTES:

OpenLDAP has been installed. You can access the server from within the k8s cluster using:
  dsp-ldap-openldap.ldap.svc.cluster.local:389

You can access the LDAP adminPassword and configPassword using:
  kubectl get secret --namespace ldap dsp-ldap-openldap -o jsonpath="{.data.LDAP_ADMIN_PASSWORD}" | base64 --decode; echo
  kubectl get secret --namespace ldap dsp-ldap-openldap -o jsonpath="{.data.LDAP_CONFIG_PASSWORD}" | base64 --decode; echo

You can access the LDAP service, from within the cluster (or with kubectl port-forward) with a command like (replace password and domain):
  ldapsearch -x -H ldap://dsp-ldap-openldap-service.ldap.svc.cluster.local:389 -b dc=example,dc=org -D "cn=admin,dc=example,dc=org" -w $LDAP_ADMIN_PASSWORD
...
```bash
# OutCluster.
minikube -n $NAMESPACE service dsp-ldap-openldap --url
# http://192.168.64.13:30059
# http://192.168.64.13:30495
curl http://192.168.64.13:30059
# curl: (52) Empty reply from server
LDAP_ADMIN_PASSWORD=$(kubectl get secret --namespace ldap dsp-ldap-openldap -o jsonpath="{.data.LDAP_ADMIN_PASSWORD}" | base64 --decode; echo)
