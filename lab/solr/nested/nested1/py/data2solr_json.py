...
def reformat_to_solr_with_path(d_original, d_solr, top_id, top_type, path):
    ...
    for k, v in d_original.iteritems():
        ...
        if path[-1] in d_original.keys() and path[-1] != "text":
            popped = path.pop()
            if len(path) < N and print_flag:
                print("popped at the beginning = {}".format(popped))
        ...
        if isinstance(v, dict):
            ...
            if path[-1] in v.keys():
                popped = path.pop()
                if len(path) < N and print_flag:
                    print("popped child at the end = {}".format(popped))
            if path[-1] == k :
                popped = path.pop()
                if len(path) < N and print_flag:
                    print("popped k at the end = {}".format(popped))
        ...
        #for list of dicts, i.e., objects
            ...
            for d_obj in v:
                if path[-1] == k:
                    popped = path.pop()
                    if len(path) < N and print_flag:
                        print("popped on cycle = {}".format(popped))
...
def main(argv):
    ...
    print('Input file is {}'.format(inputfile_nm))
    print('Output file is {}'.format(outputfile_nm))
