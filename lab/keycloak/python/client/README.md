...
export DLA_KEYCLOAK_SCHEME_HOST=http://localhost:8080
export DLA_KEYCLOAK_REALM_CLIENT_SECRET=...
```
...
keycloak_openid = KeycloakOpenID(
    server_url="{}/auth/".format(os.environ['DLA_KEYCLOAK_SCHEME_HOST']),
    realm_name="datalayer",
    client_id="datalayer",
    client_secret_key=os.environ['DLA_KEYCLOAK_REALM_CLIENT_SECRET'],
    verify=True
    )
# Get WellKnow.
config_well_know = keycloak_openid.well_know()
# Get Token.
token = keycloak_openid.token("eric", "123")
token
token = keycloak_openid.token("eric", "123", totp="012345")
# Get Userinfo.
...
jwk = json.dumps(certs['keys'][0])
public_key = RSAAlgorithm.from_jwk(jwk)
decoded = jwt.decode(token['access_token'], public_key, algorithms='RS256', options={'verify_exp': False, 'verify_signature': False, 'verify_aud': False, 'exp': False})
# Decode Token.
...
policies = keycloak_openid.get_policies(token['access_token'], method_token_info='decode', key=public_key)
