---
title: Kernel Manager
---

# Kernel Manager

## Manager

+ [jupyter_kernel_mgmt](https://github.com/takluyver/jupyter_kernel_mgmt).
+ [jupyter_ssh_kernels](https://github.com/takluyver/jupyter_ssh_kernels).
+ [jupyter_docker_kernels](https://github.com/takluyver/jupyter_docker_kernels).

+ [jupyter_kernel_test](https://github.com/takluyver/jupyter_kernel_test).

## Providers

+ [remote_kernel_provider](https://github.com/gateway-experiments/remote_kernel_provider).
+ [yarn_kernel_provider](https://github.com/gateway-experiments/yarn_kernel_provider).
+ [kubernetes_kernel_provider](https://github.com/gateway-experiments/kubernetes_kernel_provider).

## Manager in Notebook

+ [WIP: Use new kernel management APIs in notebook server](https://github.com/jupyter/notebook/pull/4170).
