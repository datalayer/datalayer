...
+ `#iam` User Management
+ `#jupyterlab` JupyterLab Datalayer
+ `#jupyterlab` JupyterLab Kernels
+ `#jupyterlab` JupyterLab Office
+ `#jupyterlab` JupyterLab Reactify
+ `#jupyterlab` JupyterLab Spark
+ `#jupyterpool` JupyterPool POC
...
+ `#kuber` Kuber commands `kuber [create,seed,join,scale,pause,hibernate,delete] --name kuber --num-workers 3 --cloud aws --auth twitter --apps [hdfs,spark]`
...
+ `#notebook` Pixiedust on JupyterLab [pixiedust/1](https://github.com/datalayer-contrib/jupyterlab-pixiedust/issues/1)
+ `#notebook` Reuse K8S IAM and RBAC
+ `#notebook` S3 API Content
...
+ `#security` JupyterHub OIDC Authenticator [oauthenticator#254]](https://github.com/jupyterhub/oauthenticator/issues/254)
+ `#security` Pluggable user token creation/validation [jupyter-server#50](https://github.com/jupyter/jupyter_server/issues/50)
+ `#social` Chat Room with Connected Users
+ `#social` Follow User
+ `#social` Post to OneNote
+ `#social` Post to User Timeline
+ `#social` Static Permalink to static version
...
+ `#volume` IPFS Volumes (see [docker-volume-ipfs](https://github.com/vdemeester/docker-volume-ipfs))
