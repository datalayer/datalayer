...
def user_loader_callback(_jwt_header, identity):
    return User(
        id = identity['id'],
...
@app.route('/api/landing/auth', methods=['GET'])
@jwt_required()
def auth():
...
@app.route('/api/landing/me', methods=['GET'])
@jwt_required()
def me():
    if get_jwt_identity():
        return jsonify({
...
@app.route('/api/landing/version', methods=["GET"])
def index():
    return """<h1>Datalayer Landing</h1>
<img src="/api/landing/res/landing.svg" width="200" />"""


@app.route('/api/landing/res/<path:path>', defaults = {'folder': 'res'})
