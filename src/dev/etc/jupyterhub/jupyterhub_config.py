...
class KeycloakMixin(OAuth2Mixin):
    ...
    oidc_server_host = os.getenv('DSP_KEYCLOAK_SCHEME_HOST')
...
c.KeycloakOAuthenticator.client_secret = os.getenv('DSP_KEYCLOAK_REALM_CLIENT_SECRET')
...
c.Spawner.environment = {
    'DSP_TWITTER_CONSUMER_KEY': os.getenv('DSP_TWITTER_CONSUMER_KEY'),
    'DSP_TWITTER_CONSUMER_SECRET': os.getenv('DSP_TWITTER_CONSUMER_SECRET'),
    'DSP_TWITTER_OAUTH_CALLBACK_URL': os.getenv('DSP_TWITTER_OAUTH_CALLBACK_URL'),
    'DSP_TWITTER_OAUTH_REDIRECT': os.getenv('DSP_TWITTER_OAUTH_REDIRECT'),
    'DSP_SOLR_ZK_HOST': os.getenv('DSP_SOLR_ZK_HOST'),
}
...
#    c.Spawner.cmd = [ os.path.join(os.environ['DLAHOME'], 'src', 'dev', 'conf', 'jupyterhub', 'launch-jupyterlab.sh') ]

def docker_spawner(c):
    c.JupyterHub.spawner_class = 'dockerspawner.DockerSpawner'
    c.DockerSpawner.image = os.getenv('DSP_JUPYTERHUB_NOTEBOOK_IMAGE')
    c.DockerSpawner.default_url = '/lab'
    c.DockerSpawner.remove_containers = True
    c.Spawner.environment = { 'DSP_GITHUB_TOKEN': os.environ['DSP_GITHUB_TOKEN']}
    from jupyter_client.localinterfaces import public_ips
    c.JupyterHub.hub_ip = public_ips()[0]
