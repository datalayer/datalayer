...
def setup(test_data_path='', dst_dir=''):
    ...
    logger.info('Syncing local folder {test_data_path} with {bucket} bucket'.format(test_data_path=test_data_path, bucket=bucket_name))
    if not test_data_path:
        logger.warning('test_data_path not defined, will use current directory')
        test_data_path = os.getcwd()
    if not os.path.isdir(test_data_path):
        logger.error('test_data_path %r not found.' % test_data_path)
        raise ValueError('test_data_path %r not found.' % test_data_path)
    all_files = []
    for root, _, files in os.walk(test_data_path):
        all_files += [os.path.join(root, f) for f in files]
    for filename in all_files:
        key = os.path.join(dst_dir, os.path.relpath(filename, test_data_path))
